...
ggplot(data = gapminder_2002) +
  ...
  aes(size = gdpPercap) +
  aes(shape = continent) +
  aes(alpha = gdpPercap) + # We've done this much #BREAK
  scale_x_log10() +  #BREAK
  scale_y_reverse(limits = c(100, 0)) +  #BREAK
  scale_color_viridis_d() +  #BREAK
  scale_shape_discrete(solid = F) +  #BREAK
  scale_size_continuous(  
    breaks = c(100, 1000, 10000, 100000),
    limits = c(0, 100000)) + #BREAK
  scale_alpha(range = c(.3, .8)) #BREAK
```

`r chunk_reveal("scales", break_type = "user")`
