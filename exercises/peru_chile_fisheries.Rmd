...
#### .right[[https://en.wikipedia.org/wiki/Fishing_industry_by_country](https://en.wikipedia.org/wiki/Fishing_industry_by_country)<br>Edited: Gina Reynolds<br>Photo Credit: 
jean wimmerlin]
...
`r chunk_reveal("making_subset")`

```{r making_subset, include=F}
# Making a subset of the data
fisheries %>% 
  filter(country %in% c("Chile", "Peru")) ->
fisheries_peru_chile

readr::write_csv(fisheries, "fisheries.csv")
readr::write_csv(fisheries_peru_chile, "fisheries_peru_chile.csv")
```
...
```{r}
fisheries <- readr::read_csv("fisheries.csv")
fisheries_peru_chile <- readr::read_csv("fisheries_peru_chile.csv")
```
