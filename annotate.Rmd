...
gapminder %>% 
  ...
  directlabels::geom_dl(aes(label = lifeExp < 60), method = "smart.grid") #ROTATE 
```
...
gapminder %>% 
  ...
  annotate(geom = "curve", arrow = arrow(length = unit(2, "mm")), x = 1970, xend = 2000, y = 23, yend = 20) + #ROTATE
  annotate(geom = "text",
           x = 1970, y = 22,
