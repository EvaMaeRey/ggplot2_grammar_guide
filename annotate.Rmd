...
`r chunk_reveal("ref_lines", break_type = "rotate", widths = c(59, 40))`
...
gapminder %>% 
  ...
  annotate(geom = "text", x = 1982, y = 10, label = "1952", angle = 90, size = 6) + #ROTATE
  ...
  annotate(geom = "curve", arrow = arrow(), curvature =-1.5, x = 1970, xend = 2000, y = 23, yend = 20) + #ROTATE
  annotate(geom = "text",
           x = 1970, y = 22,
...
`r chunk_reveal("labeling_points", break_type = "rotate", widths = c(59, 40))`
...
`r chunk_reveal("plots", widths = c(59, 40))`
...
`r chunk_reveal("fct_reorder", break_type = "rotate")`

```{r fct_reorder, include = F}
dat %>% 
  ggplot() +
  aes(x = year, y = price) + 
  geom_line(size = 1.5) + 
  aes(color = fruit) + #ROTATE
  aes(color = fct_reorder2(fruit, year, price)) #ROTATE
```
...
`r chunk_reveal("via_faceting", widths = c(59, 40))`
...
dat %>% 
  ...
  theme(panel.spacing.x =
          unit(2, "lines")) +
  # overwriting above faceting specification
  facet_wrap(~fct_reorder2(fruit, year, price))
```
...
# Interested in the toy data prep for fruit example?
--
